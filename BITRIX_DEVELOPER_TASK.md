...
```
POST https://robotender.ru/api/analyze
...
Body:
{
  ...
  "purchase_number": "{номер закупки из Saby, если известен}"
}
```

**Поле `purchase_number` (опционально, но желательно):** номер извещения/закупки на площадке.
Saby отдаёт его отдельным полем — передавайте его сюда. Тогда:
- сервис НЕ тратит запрос к модели на извлечение номера (быстрее, точнее);
- по этому номеру работает кэш: если та же закупка с тем же пакетом файлов
  уже анализировалась за последние 30 дней — ответ вернётся из кэша мгновенно
  (в callback будет `"from_cache": true`). Если состав/размер файлов изменился —
  сервис автоматически проанализирует заново.

Если поле не передать — сервис попробует определить номер сам (моделью по тексту
извещения), как и раньше. Но это менее надёжно для некоторых площадок.
...
```json
{
  ...
  "external_id": "12345",
  "status": "done",
  "purchase_number": "0173100009525000076",
  "from_cache": false,
...
`purchase_number` — номер, по которому сохранён анализ. `from_cache: true` означает,
что ответ взят из кэша (та же закупка уже анализировалась).
...
4. **purchase_number** — передавайте номер закупки из Saby, если он известен. Ускоряет и повышает точность (см. шаг 2).

5. **Формат файлов** — ZIP, 7Z, RAR, PDF, DOCX, DOC, XLSX, XLS, RTF, HTML, TXT. Рекомендация: загружать архив со всей документацией.
