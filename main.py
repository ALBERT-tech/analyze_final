...
from pipeline.extractor import extract_files, _init_converter
...
# ── Инициализация Docling (один раз при старте) ──────────────
logger.info("Инициализация Docling...")
converter = _init_converter()
logger.info("Docling готов.")
...
async def analyze(
    ...
    try:
        ...
        # ── Шаг 1+2: Распаковка + Docling ───────────────────
        extracted = extract_files(saved_paths, converter)
        ...
        logger.info(f"Контекст: {context_chars} символов, обрезан={truncated}")
        ...
        logger.info(f"API ответ получен ({len(raw_content)} символов)")
